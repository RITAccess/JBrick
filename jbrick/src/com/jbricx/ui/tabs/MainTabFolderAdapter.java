...
/**
 * Contains the methods for handling a tab folder as part of the
 * {@link MainWindow}. I'm assuming too much regarding the architecture of
 * {@link MainWindow} in order to add this listener. I'm aware this is going to
 * be a headache to refactor once it's needed. Hopefully that won't be too soon.
 * 
 * The purpose is to have a current CTabFolder and maximize/minimize/restore it
 * based on the it's parent. Parent must be a SashForm because it allows
 * resizing. All the siblings must be of type CTabFolder because the are the
 * components that have the buttons for resizing. The grand parent should also
 * be a SashForm.
 * 
 * Oh, and one more thing: Always check the listeners you're appending to a
 * particular component because of the way SWT handles the events you may end up
 * with a really undesired behavior.
 * 
 * @see CTabFolder
...
public class MainTabFolderAdapter extends CTabFolder2Adapter {
	...
	public MainTabFolderAdapter(final CTabFolder tabFolder,
			final SashForm parent) {
		super();
