...
public class JBrickEditorTabFolder extends CTabFolder implements TabFolder {
  ...
  public boolean open(String filepath) {
    ...
    if (tabIndex == -1) {
      ...
      if (file.exists()) {
        ...
        JOptionPane.showMessageDialog(null, "The file you have specified does not exits!", "File Not Found!",
  ...
  private boolean askCloseWithoutSaving(JBrickTabItem tabItem) {
    ...
    if (tabItem.getDocument().isDirty()) {
      proceed = MessageDialog.openQuestion(null, "Close without saving?", "Changes to \"" + tabItem.getText()
          + "\" has not been saved.Do you still want to proceed?");
    }
    return proceed;
  }
