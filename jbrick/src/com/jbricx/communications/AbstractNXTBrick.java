...
public abstract class AbstractNXTBrick {
  ...
  /*
   * private String joinToString(List<String> lst) { String str = "";
   * 
   * for (String s : lst) { try { if (s.split(" ").length > 1) { str += "\"" + s
   * + "\""; } else { str += s; } str += " "; } catch (NullPointerException ne)
   * {
   * 
   * } }
   * 
  ...
  public abstract void notifyAllObservers(boolean connected);
}
